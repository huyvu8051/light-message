...
public class MessageController {
    ...
    List<ConversationEntity> conversations() {
        return messageService.getNewestConversations(userCtxProvider.getUserContext().username(), new Paging(0, 10));
    }


    @PostMapping("/conversations")
    ResponseEntity<ConversationEntity> conversations(@RequestBody CreateConversationRequestDTO request){
        var conversation = messageService.createGroupChatConversation(userCtxProvider.getUserContext().username(),request);
    ...
    List<MessageDTO> messages(@PathVariable long convId) {
        return messageService.getMessages(userCtxProvider.getUserContext().username(), convId, new Paging(0, 10));
    }

    @PostMapping("/messages")
    ResponseEntity<SendMessageResponseDTO> messages(@RequestBody SendMessageRequestDTO msg) {
        messageService.sendMessage(userCtxProvider.getUserContext().username(), msg);
